...
export function expandEvent(eventType: string, payload: Payload, ctx: ExpandContext): Expansion {
  ...
  switch (eventType) {
    ...
    case 'ExperienceGained': {
      ...
      // True for any off-field award: a participant that fought and was switched back out (earning the same
      // share as the finisher), or a never-deployed member paid the innate bench Exp-Share — neither is the
      // creature on the field, so log the gain and leave the on-field XP bar alone. Only the active creature
      // drives that bar (mirrors LeveledUp.onBench).
