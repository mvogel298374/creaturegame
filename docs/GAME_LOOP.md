...
| **Treasure / Mystery** | interaction-event | ✅ implemented (**Run Economy**, `RewardRunEvent`) — roll a reward (Treasure: guaranteed gold + ≥1 item; Mystery: wildcard) via an injected supplier, credit the transient `Wallet` + `Bag`, emit `RewardGranted`, then block on `AcknowledgeRewardAsync` (client reward modal). Reward *policy* is web-layer (`RewardCalculator`), like enemy scaling — not a battle seam |
