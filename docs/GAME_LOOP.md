...
| **Evolution** | interaction-event | ✅ implemented (`BattleRunEvent.TryEvolveAsync`, in `Combat/RunEvents/`) — on a level-up, offer → ALLOW/CANCEL → morph/decline. Data/decision injected via `checkEvolution` (web `EncounterFactory` + `IEvolutionRules`); see archive |
| **Party acquisition** | interaction-event | ✅ implemented (Phase 4 Stage 1c) — the `Party` roster (`creaturegame/Creatures/Party.cs`, up to 6, held on `RunState.Party`; `Player` = the lead) and per-biome fought-species tracking exist; the Poké Center heals the whole party; a themed-draft acquisition (`AcquisitionOffered` → `ChooseAcquisitionAsync` → `AcquisitionResolution`) adds a fought-type creature to the roster |
...
| **Forced switch-on-faint** | *inside the battle loop-event* | ✅ implemented (Phase 4 Stage 3) — **not a run-loop event**: it lives inside `Battle`, which now takes the `Party` (`playerParty`). When the active creature faints and a bench member is alive, `Battle` blocks on `IBattleInput.ChooseSwitchInAsync` (a forced pick), sends the survivor in against the same enemy (`Party.SetLead` ⇒ `RunState.Player`; reset + own carried status re-applied), emits `SwitchInOffered`/`CreatureSwitchedIn`/`PartyUpdated`, and continues; the run (and the battle) end only when the whole party is down — **except** when a side fled (Roar/Whirlwind) on that same turn, where there is no foe left to send anyone in against: the faint keeps precedence, no switch is offered, and the battle ends as a loss even with a live bench. `BattleRunEvent` re-reads `s.Player` post-battle so win/loss/XP/status act on the finisher. A switched-in creature is the active creature and takes every end-of-battle effect on the same terms as the starting lead — evolution and XP/Stat-Exp are party-wide, resolved 2026-07-18 (see `TODO_ARCHIVE.md` → *Switched-in creature is the active creature* and `TODO_ARCHIVE.md` → *Innate Party XP Share*). This was the Battle-holds-party groundwork the **voluntary** in-battle switching row below built on. **Mutual KO (2026-07-28):** when the enemy *and* the active creature faint on the same turn, `Battle` still records the win (`Battle.PlayerWon`, set in the enemy-faint branch and independent of whether the finisher itself survived it) — there is no foe left, so no mid-battle switch-in fires, but post-battle `BattleRunEvent` sees `PlayerWon` and, if a bench member is alive, runs `PromoteSurvivorAsync`: a **lead-reassignment** prompt (reuses `SwitchInOffered`/`ChooseSwitchInAsync`, but emits `LeadChanged`, not `CreatureSwitchedIn` — nobody takes the field mid-battle) before the reward/draft rolls, so those act on a live lead. Only a **lone** creature (nothing to promote) still ends the run as a loss. See `TODO_ARCHIVE.md` → *Mutual KO ends the run even with a live bench* |
| **Voluntary in-battle switching** | *inside the battle loop-event* | ✅ implemented (**In-Combat Switching**, shipped 2026-07-25) — **not a run-loop event**: a fourth `SwitchTurnChoice` (sibling of FIGHT/BAG) lets the player swap the active creature on **any** turn, at the cost of the turn, resolving before every move regardless of speed (`SwitchAction` at `SwitchPriority = 7`). Blocked only by trapping (`Battle.CanSwitchTo`), never by sleep/paralysis/confusion; volatiles reset and major status persists on the outgoing creature, exactly like the forced path. Reuses the forced-switch send-in machinery (`CreatureSwitchedIn`/`PartyUpdated`); the new wire is a `SwitchRequest`/`ChooseSwitch` on the existing turn-choice handshake plus a `TurnStarted.CanSwitch` client signal. Enemy-AI switching stays a later refinement (shipped player-only). Full record in `TODO_ARCHIVE.md` → *In-Combat Switching* |
