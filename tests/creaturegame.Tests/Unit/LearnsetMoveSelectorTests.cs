...
public class LearnsetMoveSelectorTests
{
    ...
    private static Attack Move(int id, string name, int power, DamageType type,
                               DamageCategory cat = DamageCategory.Standard) =>
        new(name, name) { Id = id, BaseDamage = power, DamageType = type, DamageCategory = cat };

    private static PokemonLearnset Entry(int moveId, int level) =>
        new() { MoveId = moveId, LearnLevel = level, Generation = 1 };
    ...
    public void Learnset_CanonicalLatest_GivesMostRecentFourMovesAtLevel()
    {
        ...
        var learnset = new[]
        {
            Entry(1, 1), Entry(2, 1), Entry(3, 13), Entry(4, 20), Entry(5, 27), Entry(6, 34),
        };

        var result = LearnsetMoveSelector.Select(
            MoveSelectionStrategy.CanonicalLatest, learnset, moves, level: 50,
    ...
    public void Learnset_CanonicalLatest_ExcludesMovesAboveLevel()
    {
        ...
        var result = LearnsetMoveSelector.Select(
            MoveSelectionStrategy.CanonicalLatest, learnset, moves, level: 20,
            DamageType.Grass, null);
    ...
    public void Learnset_FewerThanFourCandidates_ReturnsAll()
    {
        ...
        var result = LearnsetMoveSelector.Select(
            MoveSelectionStrategy.WeightedSmart, learnset, moves, level: 50,
            DamageType.Grass, null, new SeededRandomSource(1));
    ...
    public void Learnset_WeightedSmart_AlwaysIncludesADamagingMove()
    {
        ...
        var learnset = new[] { Entry(1, 5), Entry(2, 1), Entry(3, 1), Entry(4, 9), Entry(5, 13), Entry(6, 17) };

        for (int seed = 0; seed < 50; seed++)
        {
            var result = LearnsetMoveSelector.Select(
                MoveSelectionStrategy.WeightedSmart, learnset, moves, level: 50,
                DamageType.Normal, null, new SeededRandomSource(seed));
    ...
    public void Learnset_WeightedSmart_SameSeed_PicksSameMoveset()
    {
        ...
        var learnset = new[] { Entry(1, 1), Entry(2, 9), Entry(3, 1), Entry(4, 5), Entry(5, 13), Entry(6, 38) };

        var a = LearnsetMoveSelector.Select(MoveSelectionStrategy.WeightedSmart, learnset, moves, 50, DamageType.Fire, null, new SeededRandomSource(42));
        var b = LearnsetMoveSelector.Select(MoveSelectionStrategy.WeightedSmart, learnset, moves, 50, DamageType.Fire, null, new SeededRandomSource(42));
    ...
    public void Learnset_WeightedSmart_FavorsStabAndPower_OverManyTrials()
    {
        ...
        var learnset = new[] { Entry(1, 38), Entry(2, 9), Entry(3, 5), Entry(4, 1), Entry(5, 13), Entry(6, 17) };
        ...
        for (int seed = 0; seed < 400; seed++)
        {
            var result = LearnsetMoveSelector.Select(
                MoveSelectionStrategy.WeightedSmart, learnset, moves, level: 50,
                DamageType.Fire, null, new SeededRandomSource(seed));
            if (result.Any(m => m.Id == 2)) countB++;
            if (result.Any(m => m.Id == 3)) countC++;
        }
